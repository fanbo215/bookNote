...
## 对核心动画的理解
* 为什么会有动画
动画的本质是一组静态的图形连续的变化，对于计算机而言，由于无法做到连续，采用离散的时间间隔来模拟，计算机刷新屏幕的周期为每秒60帧，如果屏幕每帧刷新的时候使用的是不同的图形，就会给使用者一种动画的错觉，所有的动画就是利用这种错觉来实现的
对于核心动画，它提供的一种机制，只需要用户输入一些动画的参数，剩下的动画显示就由系统来完成
这种机制是指，当用户将属性的变化，添加和删除layer，过渡动画这些信息完成后，将它们打包，由于动画不仅仅涉及到进程内的动画，因此，所有的动画渲染过程都由一个单独的进程来完成，这是一个全局的进程，负责处理所有发送过来的渲染，然后将这个打好的包通过进程间通信发送给渲染进程，渲染进程通过解包后根据动画参数生成一颗渲染的layer表现树，这颗表现树就是会根据时间的顺序显示不同的帧
用户需要输入的动画参数指能够依据它生成一颗表现layer树，根据类型可以分成三类，隐式，显示和过渡，在iOS中，由于每一种界面元素的变化都希望不要太突兀，因此，系统给它们设定了默认的动画行为，当通过修改属性值时，会触发对应的行为，用户也可以自己定义动画行为。同时，为了便于系统和用户都能调整该行为，需要制定一个行为的搜索策略，因此，首先搜索用户delegate的action，如果找到，则采用该行为，如果返回NSNULL，则停止动画搜索，并不执行动画，如果返回nil，则继续搜索actions字典，如果找到则采用该行为，如果返回nil则继续搜索style字典，如果找到则采用该行为，如果返回nil则执行defaultaction，如果找到则执行，否则停止
动画的提交时机：对于属性的修改这种默认的情况采用一个runloop周期，它将所有的修改都保存下来，在一个周期中统一提交一次；此处会有两种transation，一种是隐式的，在属性值值被修改时创建，runloop结束时提交，一种是显式，通过begin开始，通过commit结束，这种显式的可以嵌套，可以通过设置transaction的属性调整渲染参数

* 哪些情况下会触发动画，所有属性的变化都可以触发动画，添加layer和删除layer以及通过过渡都可以触发动画
* 隐式动画，显式动画，过渡动画
